...
\begin{center}
Due date: December 11, 2024, Wednesday, 23:59
\end{center}

\centerline{
    \includegraphics[height=1.8in]{../outputs/veach_ajar/VeachAjar.png}
    \includegraphics[height=1.8in]{../outputs/mytap/mytap_final.png}
}
...
\item \textbf{Interpolation:} Defines whether nearest, bilinear, or
trilinear
...
this edge pixel to avoid out-of-bounds access. Trilinear interpolation
represents mipmapping. This is an optional part of the homework, which
will provide you bonus points if you choose to implement it.
...
\item \textbf{NoiseConversion:} This element only applies to Perlin noise
...
the snake-like patterns. Otherwise, you get a more patchy noisy result.

\item \textbf{NumOctaves:} This element only applies to Perlin noise
textures. It defines how many octaves the noise function should contain
-- the number of scales of Perlin noise.
An octave is defined as the noise function evaluated at twice the
frequency but weighted with half the weight. It can be computed by the
following formula:
%
\begin{align}
s = \sum_{k = 0}^{K}2^{-k}n(2^kx, 2^ky, 2^kz),
\end{align}
%
where $K$ is the number of octaves (consider $K = 1$ if this parameter
        is not defined), $(x, y, z)$ is the position at which
noise is computed, and $s$ is the accumulated noise value across all
octaves. This is known as turbulence and can be used to create realistic
visual effects such as smoke, clouds, marble textures, etc. Here is an
example of one of our test scenes rendered using multi-scale Perlin
noise.

\centerline{
    \includegraphics[width=0.75\textwidth]{../outputs/dragon/dragon_new.png}
}
...
values in this element. For example, we shouldn't define both a normal map
...
exporting it to our file format. If your ray tracers support mipmapping
with textures, you will also get bonus points.
