...
inline bool triangle::hit(const ray& r, double tMin, double tMax, hitRecord& rec, mat4* _model) const
{
    ...
    rec.t = t;
    if(_model)
		rec.p = glm::vec3((*_model * vec4(r.at(rec.t), 1.0f)));
    else
		rec.p = r.at(rec.t);


    vec3 outwardNormal = glm::normalize(cross(edge1, edge2));  // Normal of the triangle\

    rec.setFaceNormal(r, outwardNormal);
    if(_model)
		rec.normal = glm::normalize(glm::vec3((_model->transpose().inverse()) * vec4(outwardNormal, 0.0f)));
...
inline bool triangle::hit(const ray& r, double tMin, double tMax, hitRecord& rec, mat4* _model) const {
    ...
    if(_model)
    {
		//rec.normal = glm::normalize(glm::vec3(( glm::inverse(glm::transpose(*_model))) * vec4(normal, 0.0f)));
		rec.normal = glm::normalize(glm::vec3(( glm::inverse(glm::transpose(*_model))) * vec4(normal, 0.0f)));
        //rec.setFaceNormal(r, normal);
		//if (!rec.frontFace)
			//rec.normal = -rec.normal;

		rec.tangent = glm::normalize(glm::vec3(*_model * vec4(dpdu, 0.0f)));
		rec.bitangent = glm::normalize(glm::vec3(*_model * vec4(dpdv, 0.0f)));

		rec.dpdu = (glm::vec3(*_model * vec4(dpdu, 0.0f)));
		rec.dpdv = (glm::vec3(*_model * vec4(dpdv, 0.0f)));

		if (glm::dot(glm::cross(rec.dpdv, rec.dpdu), rec.normal) < 0.0f)
		{
			rec.dpdu = -rec.dpdu;
		}

		//rec.normal = glm::normalize(glm::cross(rec.dpdv, rec.dpdu));

		if (glm::determinant(*_model) < 0.0f)
		{
			//rec.normal = -rec.normal;
		}
		...
		tv0 = glm::vec3(*_model * vec4(tv0, 1.0f));
		tv1 = glm::vec3(*_model * vec4(tv1, 1.0f));
		tv2 = glm::vec3(*_model * vec4(tv2, 1.0f));

    	vec3 tv0v1 = tv1 - tv0;
		vec3 tv0v2 = tv2 - tv0;
		rec.normal = glm::normalize(cross(tv0v1, tv0v2));
		...
		if (determinant < 0.1f && determinant > -0.1f) {
			   CoordinateSystem(glm::normalize(cross(p3 - p1, p2 - p1)), rec.dpdu, rec.dpdv);
